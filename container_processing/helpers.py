...
class CachingKojiWrapper(KojiWrapperBase):
    ...
    def _cross_populate_cache(self):
        for build_id in self._build_data:
            val = self._build_data[build_id]
            if val['nvr'] not in self._build_id_or_nvr_to_build_id:
                self._build_id_or_nvr_to_build_id[val['nvr']] = build_id
        ...
        for task_id in self._task_results:
            ...
            if 'repositories' in val and 'koji_builds' in val:
                for build_id in val['koji_builds']:
                    if int(build_id) not in self._build_id_to_build_task_id:
    ...
    def save_cache(self, path=CACHE_PATH, debug=False):
        ...
        for filename, cache in [('build_id_to_parent_id', self._build_id_to_parent_id),
                                ...
                                ('build_id_or_nvr_to_build_id', self._build_id_or_nvr_to_build_id)]:
    ...
    def getBuildId(self, build_id_or_nvr):
        ...
        if build_id is None and build_id_or_nvr in self._build_id_or_nvr_to_build_id:
            build_id = self._build_id_or_nvr_to_build_id[build_id_or_nvr]

        if build_id is None:
            build_id = self.getBuildData(build_id_or_nvr)['id']

        return build_id

    def getBuildData(self, build_id_or_nvr):
        ...
        if (build_id is None or build_id not in self._build_data):
            builddata = self.session.getBuild(build_id_or_nvr)
            build_id = builddata['id']

            self._build_data[build_id] = builddata
            self._build_id_or_nvr_to_build_id[builddata['nvr']] = builddata['id']
    ...
    def getTaskResults(self, task_id):
        ...
        if task_id not in self._task_results:
            self._task_results[task_id] = self.session.getTaskResult(task_id)
    ...
    def getRecordForBuild(self, build_id_or_nvr, grab_build_task_info=False):
        ...
        ret_data = {
            ...
            'parent_build_id': self._build_id_to_parent_id[build_id]
        }

        if grab_build_task_info:
            task_results = self.getTaskResults(build_task_id)
    ...
    def get_container_builds_from_koji_tag(self, koji_tag, get_extra_info=False):
        ...
        for build in koji_tag.builds():
            if build['nvr'] not in self._build_id_or_nvr_to_build_id:
                self._build_id_or_nvr_to_build_id[build['nvr']] = build['id']
